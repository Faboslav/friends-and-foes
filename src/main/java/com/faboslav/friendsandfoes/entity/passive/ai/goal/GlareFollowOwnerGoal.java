...
public class GlareFollowOwnerGoal extends Goal
{
    ...
    public GlareFollowOwnerGoal(GlareEntity glare, double speed, float minDistance, float maxDistance, boolean leavesAllowed) {
        this.glare = glare;
    ...
    private boolean tryTeleportTo(int x, int y, int z) {
        if (Math.abs((double)x - this.owner.getX()) < 2.0D && Math.abs((double)z - this.owner.getZ()) < 2.0D) {
            ...
            this.glare.refreshPositionAndAngles((double)x + 0.5D, (double)y, (double)z + 0.5D, this.glare.getYaw(), this.glare.getPitch());
            this.navigation.stop();
            return true;
        }
    }
